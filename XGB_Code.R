...
setwd("~/work/fintech/kaggle/GiveMeSomeCredit/code/Kaggle-Code")
train<-read.csv("../../data/cs-training.csv") # Import train Data
test<-read.csv("../../data/cs-test.csv") # Import Test data
sample_submission<-read.csv("../../data/sampleEntry.csv")
names(train)
names(test)
...
clf <- xgb.cv(params              = param, 
              data                = all_train, 
              nrounds             = 2000,#2000, #300, #280, #125, #250, # changed from 300
              verbose             = 1,
              #early.stop.round    = 40,
              #watchlist           = watchlist,
              maximize            = FALSE,
...
clf_best <- xgboost(params        = param, 
                    data                = all_train, 
                    nrounds             = 420,# 420, #300, #280, #125, #250, # changed from 300
                    verbose             = 1,
